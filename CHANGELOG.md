...
- Ship a native **Codex plugin** (fork-only), superseding the deferral noted alongside the Claude Code plugin in this same release.

  That deferral rested on `.codex-plugin/plugin.json` accepting `skills` only as a single path string, which cannot express "ship `engineering/` and `productivity/`, exclude everything else" for a bucketed repo. ADR 0002 set a revisit condition — a `skills` array, or symlinks surviving install. Verified against `codex-cli 0.144.3`, the array condition is now met:
  ...
  `.codex-plugin/plugin.json` points `skills` at the two promoted bucket paths, and a sibling `.agents/plugins/marketplace.json` makes the repo its own single-plugin Codex marketplace:

  ```
  codex plugin marketplace add mattpocock/skills
  codex plugin add skills@alandy-skills
  ```

  Pointing at buckets rather than individual skill directories means the manifest cannot drift when a skill is added — unlike `.claude-plugin/plugin.json`, whose `skills` array lists each skill. Neither of ADR 0002's rejected workarounds is needed: `skills/` is not restructured and no flat duplicate is committed. Caveat: Codex copies the whole plugin tree into its cache, so non-promoted buckets land on disk while registering no skills. See `.agents/adr/0003-ship-a-native-codex-plugin.md`.
